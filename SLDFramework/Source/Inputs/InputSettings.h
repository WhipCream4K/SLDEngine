...
namespace SLD
{
	...
	public:
		...
		float GetInputAxis(const std::string& groupName,uint8_t* playerIndex = nullptr);
		InputEvent GetInputState(const std::string& groupName,uint8_t* playerIndex = nullptr);
		...
		void ParseMessage(const InputParams::MessageBus& message) const;
		...
		template<typename FuncPtr,
			...
			void RegisterActionCommand(
				...
				InputEvent ie,
				FuncPtr fnPtr,
				const SharedPtr<Object>& objInstance,
				bool** outHandle,
				Args&&... args);
	...
	void InputSettings::RegisterActionCommand(
		const std::string& groupName,
		InputEvent ie,
		FuncPtr fnPtr,
		const SharedPtr<Object>& objInstance,
		...
		Args&&... args)
	{
		...
		//SharedPtr<CallbackType> smartEv{ ev,[&loggerResource,&commandTable](CallbackType* ptr)
		//{
	...
	void InputSettings::RegisterAxisCommand(
		const std::string& groupName,
		FuncPtr fnPtr,
		const SharedPtr<Object>& objInstance,
		float** outHandle)
	{
		...
		//// This smart pointer does nothing because our ptr is already on the resources
		//SharedPtr<CallbackType> smartEv{ ev,[&resource](CallbackType* ptr)
		//{
	...
	template <typename FuncPtr, typename Object, typename ... Args>
	void InputSettings::CreateActionCommand(const std::string& actionGroupName,
		...
		Args&&... args)
	{
		...
		//SharedPtr<CallbackType> smartEv{ ev,[&logResource](CallbackType* ptr)
		//{
	...
	template <typename FuncPtr, typename Object>
	void InputSettings::CreateAxisCommand(const std::string& axisGroupName, WeakPtr<GameObject> reference,
